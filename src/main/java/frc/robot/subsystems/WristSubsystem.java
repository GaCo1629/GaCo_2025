...
public class WristSubsystem extends SubsystemBase {
	...
	public WristSubsystem() {
		...
		angleTrapezoidProfile = new TrapezoidProfile(new Constraints(
				Constants.Wrist.kAngleMaxVelocity.in(DegreesPerSecond),
		...
		intakeConfig.idleMode(IdleMode.kBrake).smartCurrentLimit(50);
		intakeConfig
				.encoder
	...
	// The configuration interfaces may be accessed by typing in the IP address of the roboRIO into
	// a web
	...
	public boolean inPosition() {
		Globals.WRIST_IN_POSITION =
				(Math.abs(angleGoal.position - getWristAngle().in(Degrees)) < Constants.Wrist.kAngleTollerance);
