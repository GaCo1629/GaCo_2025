...
public class TowerSubsystem extends SubsystemBase {
		...
		switch (currentState) {
			...
			case WAITING_FOR_ALGAE: {
				if (isTriggered(TowerEvent.SCORE)) {
					...
					// wrist.setIntakeSpeed(Constants.WristConstants.kLowAlgaeIntakePower);  //
					// hold lightly
	...
	private Boolean isHoldingGoTo() {
		if ((pendingEvent == TowerEvent.GOTO_L1)
				|| (pendingEvent == TowerEvent.GOTO_L2)
				|| (pendingEvent == TowerEvent.GOTO_L3)
				|| (pendingEvent == TowerEvent.GOTO_L4)) {
			return true;
		} else {
			return false;
		}
