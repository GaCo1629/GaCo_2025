...
public class Telemetry {
	...
	private final DoubleArrayPublisher fieldPub =
			table.getDoubleArrayTopic("Robot").publish();
	...
	private static final double AUTO_ANIMATION_SPEED =
			0.4; // Speed of the animation. Default = 0.25 *Animation speed not to scale with the
	// actual autonomous speed
	private static final int AUTO_ANIMATION_FREEZE_TIME =
			10; // How long to wait before restarting the animation. Default = 10
	...
	public static void displayAutoPaths() {
		...
		try {
			paths = PathPlannerAuto.getPathGroupFromAutoFile(
					SmartDashboard.getEntry("Auto Mode/active").getString("None"));
		} catch (IOException e) {
		...
		if (paths != null && !paths.equals(pathsLast)) {
			...
			if (paths != null && paths.size() > 0) {
				...
				for (int i = 0; i < paths.size(); i++) {
					poses.addAll(paths.get(i).getAllPathPoints().stream()
				...
				for (int i = 0; i < paths.size(); i++) {
					rotations.addAll(paths.get(i).getAllPathPoints().stream()
							.map(point -> point.rotationTarget)
							.collect(Collectors.toList()));
				}

				// Add rotations to the poses in each path by interpolating values between start and
				// end rotation
				...
				for (int i = 0; i < poses.size(); i++) {
					...
					if (rotations.get(i) != null) {
						RotationTarget tempRot =
								rotations.get(i); // Current rotational value (null unless at start or end
						// of a path / at rotation checkpoint)
						...
						if (tempRot != null && tempRot.position() != 0) {
							...
							if (startPoint == 0) {
								poses.set(
										startPoint,
										new Pose2d(
												poses.get(startPoint).getTranslation(),
												new Rotation2d((paths.get(0)
														.getIdealStartingState()
														.rotation()
														.getRadians()))));
								toRotate = goalAngle
										- poses.get(startPoint).getRotation().getRadians();
								...
								poses.set(
										...
										new Pose2d(
												poses.get(startPoint).getTranslation(),
												new Rotation2d((toRotate / (i - (startPoint - 1)))
														+ poses.get(startPoint - 1)
																.getRotation()
																.getRadians())));
							}
							...
							for (int j = startPoint + 1; j <= i; j++) {
								poses.set(
										j,
										new Pose2d(
												poses.get(j).getTranslation(),
												new Rotation2d((toRotate / (i - (startPoint - 1)))
														+ poses.get(j - 1)
																.getRotation()
																.getRadians())));
							}
							startPoint = i + 1; // Update the starting point to the next
							// uninitialized one
						}
					}
				}
		...
		if (poses != null && firstAnimation) {
			...
			if (autoAnimationStep >= poses.size() - 1) {
				autoAnimationStep = -AUTO_ANIMATION_FREEZE_TIME; // Wait this many cycles before restarting the
				// animation
