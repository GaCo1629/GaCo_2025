...
public class RobotContainer {
	...
	private final AngularVelocity MaxAngularRate = RadiansPerSecond.of(
			Units.rotationsToRadians(0)); // 3/4 of a rotation per second max angular velocity in radians per
	// second

	/* Setting up bindings for necessary control of the swerve drive platform */
	private final SwerveRequest.FieldCentric drive = new SwerveRequest.FieldCentric()
			.withDeadband(MaxSpeed.times(DriverConstants.kDriveDeadband))
			.withRotationalDeadband(MaxAngularRate.times(DriverConstants.kRotationDeadband)) // Add a 8% deadband
	...
	static final Transform3d robotToLowerCam =
			new Transform3d(new Translation3d(0.26, 0.00, 0.20), new Rotation3d(0, 0, 0));
	static final Vector<N3> lowerCamStdDevs = VecBuilder.fill(0.5, 0.5, Units.degreesToRadians(5));

	static final Transform3d robotToUpperCam =
			new Transform3d(new Translation3d(-0.05, 0.00, 1.017), new Rotation3d(0, Math.toRadians(2.1), Math.PI));
	...
	public final VisionSubsystem lowerVision =
			new VisionSubsystem(drivetrain, "LowerTagCamera", robotToLowerCam, lowerCamStdDevs);
	...
	public RobotContainer() {
		...
		NamedCommands.registerCommand(
				"WAIT_FOR_LOWERING", new WaitForTowerStateCmd(tower, TowerState.PAUSING_AFTER_SCORING_CORAL));
		...
		SmartDashboard.putData("Swerve Drive", new Sendable() {
			...
			public void initSendable(SendableBuilder builder) {
				...
				builder.addDoubleProperty(
						"Front Left Angle",
						() -> drivetrain.getModule(0).getPosition(false).angle.getRadians(),
						null);
				builder.addDoubleProperty(
						"Front Left Velocity",
						() -> drivetrain
				...
				builder.addDoubleProperty(
						"Front Right Angle",
						() -> drivetrain.getModule(1).getPosition(false).angle.getRadians(),
						null);
				builder.addDoubleProperty(
				...
				builder.addDoubleProperty(
						"Back Left Angle",
						() -> drivetrain.getModule(2).getPosition(false).angle.getRadians(),
						null);
				builder.addDoubleProperty(
						"Back Left Velocity",
						() -> drivetrain
				...
				builder.addDoubleProperty(
						"Back Right Angle",
						() -> drivetrain.getModule(3).getPosition(false).angle.getRadians(),
						null);
				builder.addDoubleProperty(
						"Back Right Velocity",
						() -> drivetrain
				...
				builder.addDoubleProperty(
						"Robot Angle",
						() -> drivetrain.getRotation3d().getMeasureAngle().baseUnitMagnitude(),
						null);
			}
		});
		...
		drivetrain.setDefaultCommand(
				...
				drivetrain.applyRequest(
						() -> drive.withVelocityX(MaxSpeed.in(MetersPerSecond)
										* -joystick.getLeftY()
										* Constants.DriverConstants.kMaxDriveSpeedPercentage.in(Percent)
										* tower.getTowerSpeedSafetyFactor().in(Percent)) // Drive forward
								// with negative Y
								// (forward)
								.withVelocityY(MaxSpeed.in(MetersPerSecond)
										* -joystick.getLeftX()
										* Constants.DriverConstants.kMaxDriveSpeedPercentage.in(Percent)
										* tower.getTowerSpeedSafetyFactor().in(Percent)) // Drive left with
								// negative X
								// (left)
								.withRotationalRate(MaxAngularRate.in(RadiansPerSecond)
										* -joystick.getRightX()
										* Constants.DriverConstants.kMaxTurnSpeedPercentage.in(Percent)
										* tower.getTowerSpeedSafetyFactor().in(Percent)) // Drive
	...
	private void configureBindings() {
		...
		joystick.rightTrigger(0.5)
				.onTrue(tower.runOnce(() -> tower.triggerEvent(TowerEvent.SCORE))); // score coral or algae
		...
		joystick.leftTrigger(0.5)
				.onTrue(approach.runOnce(() -> approach.startApproach()))
				.onFalse(drivetrain.runOnce(() -> drivetrain.applyRequest(
						() -> forwardStraight.withVelocityX(0.0).withVelocityY(0.0))));
		...
		joystick.pov(180).whileTrue(drivetrain.applyRequest(() -> forwardStraight
				.withVelocityX(xSpeed.in(MetersPerSecond) * -1.0)
				.withVelocityY(0)));
		joystick.pov(90)
				.whileTrue(drivetrain.applyRequest(
		...
		copilot_1
				.button(DriverConstants.reset)
				.onTrue(
						lowerVision.runOnce(() -> lowerVision.setSafetyOverride(true))
						//
						// .andThen(upperVision.runOnce(() -> upperVision.setSafetyOverride(true)))
						// //   only override low cam safety to reposition
						);
		...
		copilot_1
				.button(DriverConstants.pose_i)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_I)));
		copilot_1
				.button(DriverConstants.pose_j)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_J)));
		copilot_1
				.button(DriverConstants.pose_ija)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_IJ)));
		copilot_1
				.button(DriverConstants.pose_k)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_K)));
		copilot_1
				.button(DriverConstants.pose_l)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_L)));
		copilot_1
				.button(DriverConstants.pose_kla)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_KL)));

		// ===  CoPilot 2 Buttons  ===========================================

		copilot_2.button(DriverConstants.reset).onTrue(tower.runOnce(() -> tower.triggerEvent(TowerEvent.HOME_TOWER)));
		copilot_2
				.button(DriverConstants.pose_a)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_A)));
		copilot_2
				.button(DriverConstants.pose_b)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_B)));
		copilot_2
				.button(DriverConstants.pose_aba)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_AB)));
		copilot_2
				.button(DriverConstants.pose_c)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_C)));
		copilot_2
				.button(DriverConstants.pose_d)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_D)));
		copilot_2
				.button(DriverConstants.pose_cda)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_CD)));
		copilot_2
				.button(DriverConstants.pose_e)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_E)));
		copilot_2
				.button(DriverConstants.pose_f)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_F)));
		copilot_2
				.button(DriverConstants.pose_efa)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_EF)));
		copilot_2
				.button(DriverConstants.pose_g)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_G)));
		copilot_2
				.button(DriverConstants.pose_h)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_H)));
		copilot_2
				.button(DriverConstants.pose_gha)
				.onTrue(tower.runOnce(() -> approach.identifyTarget(ApproachTarget.REEF_GH)));
	...
	MutAngle robotRotation =
			Degrees.mutable(drivetrain.getState().Pose.getRotation().getDegrees());
	...
	public void faceCoralStation(boolean isLeft) {
		...
		approachCoralStationCommand = drivetrain
				.applyRequest(
						() -> drive.withVelocityX(
										MaxSpeed.in(MetersPerSecond) * -joystick.getLeftY() / 2.0) // Drive forward with
								// negative Y (forward)
								.withVelocityY(MaxSpeed.in(MetersPerSecond)
										* -joystick.getLeftX()
										/ 2.0) // Drive left with negative X
								// (left)
								// MaxAngularrate * headingError / 80 =
								// angularVelocity
								.withRotationalRate(clampAngularVelocity(
										MaxAngularRate.in(RadiansPerSecond) * headingError.in(Radians) / 80.0,
										-Math.PI,
										Math.PI,
										Math.PI / 4.0,
										RadiansPerSecond)) // Auto rotate to
						// position
						)
	...
	private AngularVelocity clampAngularVelocity(
			double value, double min, double max, double innerBound, AngularVelocityUnit unit) {
		if (value > max) {
			value = max;
